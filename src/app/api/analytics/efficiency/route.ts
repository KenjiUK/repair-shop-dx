...
import {
  ...
  generateEfficiencyAnalyticsData,
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 業務効率分析データを生成
    const efficiencyData = generateEfficiencyAnalyticsData(
      allJobsResult.data,
      startDate,
      endDate
    );
    ...
    return NextResponse.json({
      success: true,
      data: efficiencyData,
    } as ApiResponse<EfficiencyAnalyticsData>);
  } catch (error) {
    console.error("[Analytics Efficiency] エラー:", error);
    return createErrorResponse(
      ErrorCodes.INTERNAL_ERROR,
      error instanceof Error ? error.message : "業務効率分析データの取得に失敗しました",
      500
    );
  }
}
