...
/**
 * 12ヶ月点検・24ヶ月点検 再設計版 テストページ
 * 
 * 実装中のコンポーネントを確認するためのテストページ
 */
...
export default function InspectionRedesignTestPage() {
  const [type, setType] = useState<"12month" | "24month">("12month");
  const [items, setItems] = useState<InspectionItemRedesign[]>(() =>
    getInspectionItems("12month")
  );
  ...
  const mockJob: ZohoJob = {
    ...
    field12: null,
    tagId: null,
    serviceKind: type === "24month" ? "車検" : "12ヵ月点検",
  ...
  const diagnosisTitle = type === "24month" ? "24ヶ月点検（車検）診断" : "12ヶ月点検診断";
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 dark:bg-slate-800 dark:border-slate-700">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <span className="text-base text-slate-700 dark:text-white">点検タイプ:</span>
          <Button
            variant={type === "12month" ? "default" : "outline"}
            onClick={() => setType("12month")}
            className="h-16 text-lg"
          >
            12ヶ月点検
          </Button>
          <Button
            variant={type === "24month" ? "default" : "outline"}
            onClick={() => setType("24month")}
            className="h-16 text-lg"
          >
            24ヶ月点検（車検）
          </Button>
      ...
      <AppHeader
        ...
        pageTitleIcon={<Activity className="h-5 w-5 text-slate-700 shrink-0 dark:text-white" />}
      >
        {/* ページタイトル */}
        <div className="mb-3">
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 dark:text-white">
            <Activity className="h-6 w-6 text-slate-700 shrink-0 dark:text-white" />
            {diagnosisTitle}
          </h1>
        </div>
      ...
      {/* メインコンテンツ */}
      <main className="max-w-4xl mx-auto px-4 py-6 pb-32 bg-slate-50 dark:bg-slate-900" style={{ paddingTop: 'calc(var(--header-height, 176px) + 1.5rem)' }}>
        <div className="space-y-6">
          {/* 走行距離入力欄（独立セクション） */}
          <Card className="border-slate-200 shadow-md hover:shadow-lg transition-shadow dark:border-slate-700 dark:bg-slate-800">
          ...
          {blogPhotos.length > 0 && (
            <Card className="border-slate-200 shadow-md hover:shadow-lg transition-shadow dark:border-slate-700 dark:bg-slate-800">
              <CardHeader className="pb-3">
                <CardTitle className="text-xl font-semibold flex items-center gap-2 dark:text-white">
                  <Camera className="h-6 w-6 text-slate-600 shrink-0 dark:text-white" />
                  ブログ撮影用写真
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="grid grid-cols-3 gap-2">
                  {blogPhotos.map((photo, index) => (
                    <div key={photo.fileId} className="relative aspect-square rounded-lg overflow-hidden border-2 border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
                      <Image
                        src={photo.url}
                        alt={photo.fileName}
                        fill
          ...
          <Card className="border-slate-200 shadow-md hover:shadow-lg transition-shadow dark:border-slate-700 dark:bg-slate-800">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-xl font-semibold text-slate-900 dark:text-white">
                <NotebookPen className="h-6 w-6 text-slate-600 shrink-0 dark:text-white" />
                作業メモ
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="flex items-start gap-2">
                <Textarea
                  ...
                  className="text-lg flex-1 resize-none min-h-[140px]"
                  rows={5}
                />
                <VoiceInputButton
          ...
          <Card className="border-slate-200 shadow-md hover:shadow-lg transition-shadow dark:border-slate-700 dark:bg-slate-800">
            <CardHeader className="pb-3">
              <CardTitle className="text-xl font-semibold flex items-center gap-2 dark:text-white">
                <Lightbulb className="h-6 w-6 text-slate-600 shrink-0 dark:text-white" />
                整備アドバイス
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
          ...
          {/* プレビューと点検完了ボタン */}
          <div className="flex gap-4 pt-4 border-t border-slate-200 dark:border-slate-700 dark:bg-slate-900">
            <Button
              variant="outline"
              onClick={handlePreview}
              className="flex-1 h-16 text-xl font-medium gap-2 dark:border-slate-600 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700"
            >
            ...
            <Button
              ...
              className="flex-1 h-16 text-xl font-medium gap-2 dark:bg-blue-600 dark:text-white dark:hover:bg-blue-700"
            >
