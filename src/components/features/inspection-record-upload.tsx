...
import { uploadInspectionRecord } from "@/lib/inspection-record-upload";
...
interface InspectionRecordUploadProps {
  /** 顧客ID */
  customerId: string;
  /** 顧客名 */
  customerName: string;
...
export function InspectionRecordUpload({
  customerId,
  customerName,
  vehicleId,
  vehicleName,
  ...
  const handleUpload = async () => {
    ...
    try {
      const result = await uploadInspectionRecord(
        file,
        customerId,
        customerName,
        vehicleId,
      ...
      {file && !uploadedFileId && (
        <div className="space-y-3">
          <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg border">
            {previewUrl ? (
              <div className="relative w-32 h-32 rounded border overflow-hidden shrink-0">
                <Image
                  src={previewUrl}
                  alt="自動車検査証記録事項プレビュー"
                  fill
              ...
              <div className="w-32 h-32 flex items-center justify-center bg-slate-200 rounded border shrink-0">
