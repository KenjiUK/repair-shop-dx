    ...
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          // 位置とサイズ（スマホ対応）
          "fixed left-[50%] z-50 grid w-full max-w-[calc(100%-1rem)] translate-x-[-50%]",
          "top-[50%] translate-y-[-50%] sm:top-[50%] sm:translate-y-[-50%]",
          // 最大高さとスクロール（スマホ対応）
          "max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-4rem)]",
          "overflow-y-auto",
          // パディング（スマホで小さく、PCで大きく）
          "p-4 sm:p-6",
          // その他のスタイル
          "gap-4 rounded-lg border shadow-lg duration-200 outline-none",
          "sm:max-w-lg",
          className
        )}
        {...props}
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            className={cn(
              ...
              // タッチターゲットサイズ（40歳以上ユーザー向け、最小48px × 48px）
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn("text-muted-foreground text-base", className)} // text-sm → text-base (40歳以上ユーザー向け、DESIGN_SYSTEM.md準拠)
      {...props}
    />
  )
}
