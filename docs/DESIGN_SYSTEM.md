...
5. [アイコン](#5-アイコン)
6. [ボタン](#6-ボタン)
   - [6-6. 診断・作業ページ向けボタンサイズ（特別ルール）](#6-6-診断作業ページ向けボタンサイズ特別ルール)
7. [フォーム要素](#7-フォーム要素)
   - [7-1-1. 診断・作業ページ向け入力フィールド（特別ルール）](#7-1-1-診断作業ページ向け入力フィールド特別ルール)
   - [7-5. 見積行のUI仕様](#7-5-見積行のui仕様)
8. [バッジ](#8-バッジ)
9. [カード](#9-カード)
10. [カラーシステム](#10-カラーシステム)
11. [ダークモード対応](#11-ダークモード対応)
12. [レスポンシブデザイン](#12-レスポンシブデザイン)
13. [メッセージボックス（Alert/Toast/Dialog）](#13-メッセージボックスalerttoastdialog)
14. [アクセシビリティ](#14-アクセシビリティ)
15. [ヘッダー・ナビゲーション](#15-ヘッダーナビゲーション)
16. [アニメーション](#16-アニメーション)
17. [TOPページの詳細デザインシステム](#17-toppageの詳細デザインシステム)
18. [実装チェックリスト](#18-実装チェックリスト)
19. [禁止事項と置き換え方法](#19-禁止事項と置き換え方法)
20. [Sheet（サイドパネル）](#20-sheetサイドパネル)
21. [スケルトン（ローディング状態）](#21-スケルトンローディング状態)
22. [Dialog（ダイアログ）の詳細仕様](#22-dialogダイアログの詳細仕様)
23. [検索バー](#23-検索バー)
24. [エラー表示](#24-エラー表示)
25. [ローディング状態](#25-ローディング状態)
26. [トースト通知の詳細仕様](#26-トースト通知の詳細仕様)
27. [フォームバリデーション](#27-フォームバリデーション)
28. [業務フローとユーザーロール](#28-業務フローとユーザーロール)
29. [文言（ラベル・メッセージ）の統一ルール](#29-文言ラベルメッセージの統一ルール)
30. [音声入力](#30-音声入力)
31. [Avatar（アバター）](#31-avatarアバター)
32. [Progress（プログレスバー）](#32-progressプログレスバー)
33. [Tabs（タブ）](#33-tabsタブ)
34. [Separator（区切り線）](#34-separator区切り線)
35. [ScrollArea（スクロール領域）](#35-scrollareaスクロール領域)
36. [DropdownMenu（ドロップダウンメニュー）](#36-dropdownmenuドロップダウンメニュー)
37. [AlertDialog（確認ダイアログ）](#37-alertdialog確認ダイアログ)
...
## 6-6. 診断・作業ページ向けボタンサイズ（特別ルール）

**対象ページ**: `/mechanic/diagnosis/[id]`、`/mechanic/work/[id]`  
**理由**: 40歳以上の整備士が現場で作業手袋を着用してスマートフォンを使用するため、より大きなタッチターゲットが必要

### 6-6-1. ボタンサイズ

| サイズ | 高さ | Tailwindクラス | 用途 | テキストサイズ | アイコンサイズ |
|--------|------|----------------|------|--------------|--------------|
| **診断・作業ページ用** | 64px | `h-16` | 診断・作業ページの主要ボタン | `text-xl` (20px) | `h-6 w-6` (24px) |
| **診断・作業ページ用（その他）** | 64px | `h-16` | 診断・作業ページのその他ボタン | `text-base` (16px) | `h-6 w-6` (24px) |

### 6-6-2. 実装例

```typescript
// 診断・作業ページの主要ボタン（良好ボタンなど）
<Button className="w-full h-16 text-xl font-semibold bg-green-600 hover:bg-green-700 text-white">
  <CheckCircle2 className="h-6 w-6 shrink-0" />
  良好（レ）
</Button>

// 診断・作業ページのその他ボタン
<Button className="h-16 text-base font-semibold border-2 border-slate-300">
...
以下のコンポーネントで適用：
- `InspectionStatusBottomSheet` - 検査ステータス選択ボトムシート
- `InspectionMeasurementInput` - 測定値入力フィールド
- 診断・作業ページ内のその他のボタン

**注意**: この特別ルールは診断・作業ページ専用です。他のページでは通常のボタンサイズ（`h-12`または`h-14`）を使用してください。
...
### 7-1-1. 診断・作業ページ向け入力フィールド（特別ルール）

**対象ページ**: `/mechanic/diagnosis/[id]`、`/mechanic/work/[id]`  
**理由**: 40歳以上の整備士が現場で作業手袋を着用してスマートフォンを使用するため、より大きな入力フィールドが必要

| プロパティ | 値 | Tailwindクラス | 説明 |
|-----------|-----|----------------|------|
| **高さ** | 64px | `h-16` | 作業手袋対応のタッチターゲットサイズ |
| **テキストサイズ** | 20px | `text-xl` | 読みやすさを重視（作業環境） |
| **パディング** | 標準 | `px-3 py-2` | - |
| **ボーダー** | 1px | `border border-slate-300` | - |
| **角丸** | 6px | `rounded-md` | - |
...
**注意**: この特別ルールは診断・作業ページ専用です。他のページでは通常の入力フィールドサイズ（`h-12 text-base`）を使用してください。
...
## 13. メッセージボックス（Alert/Toast/Dialog）
...
| 内部コード | UI表示 | 説明 | バッジ色 |
|-----------|--------|------|---------|
| `required` | 「今回絶対必要」 | 安全上・法規上、今回の整備で必須の作業 | `bg-red-600` |
| `recommended` | 「やったほうがいい」 | 整備士として推奨する作業 | `bg-emerald-600` |
| `optional` | 「お客さん次第」 | お客様の判断に委ねる作業 | `bg-slate-500` |

**注意事項:**
- 型定義（`EstimatePriority`）は後方互換性のため `'required' | 'recommended' | 'optional'` のまま維持
- UI表示のみ新しい用語を使用
- 旧用語「必須整備」「推奨整備」「任意整備」は使用禁止（コードコメントでの参照は可）
...
- [ダークモードカラーリングガイドライン](./DARK_MODE_COLORING_GUIDELINES.md) - ダークモード対応の詳細ガイドライン
