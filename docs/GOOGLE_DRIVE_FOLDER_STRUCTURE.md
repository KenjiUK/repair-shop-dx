...
1. **仕様書**: `01_入庫写真/`, `02_証拠写真/`, `03_作業報告/` というサブフォルダが想定されている
2. **実装**: Work Orderフォルダ（`wo-{workOrderId}/`）内に直接保存されている可能性が高い

### 確認が必要な点

現在の実装では、Work Orderフォルダ内に以下のサブフォルダが**明示的に作成されているか不明**です：

- `01_入庫写真/` - 入庫時の4点撮影写真（画像ファイルのみ）
- `02_証拠写真/` - 診断時の証拠写真・動画（不具合箇所の写真、異音などの動画）
- `03_作業報告/` - 作業完了後のAfter写真（画像ファイルのみ）
...
1. **現在の実装を確認**: Work Orderフォルダ内にサブフォルダが作成されているか確認
2. **ファイル命名関数の実装**: 提案されている命名規則に基づいてファイル名を生成する関数を作成
3. **フォルダ構造の統一**: 仕様書に合わせてサブフォルダ構造を実装（推奨）
4. **動画ファイルの命名規則**: 動画ファイルも写真と同じ命名規則を適用
