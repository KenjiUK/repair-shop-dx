...
### 10. ダークモード切り替えボタン ❌ 一時的に削除（再実装予定）
**削除日:** 2025-01-XX

**現在の状態:**
- ヘッダーから一時的に削除済み
- `ThemeToggleButton`コンポーネントは存在（`src/components/features/theme-toggle-button.tsx`）
- 詳細は`docs/DARK_MODE_TOGGLE_FUTURE_PLAN.md`を参照

**実装時の考慮事項:**
- モバイルでの表示位置を検討（右上が混雑しないように）
- アクセシビリティの確保（キーボード操作、スクリーンリーダー対応）
- ユーザーの設定をローカルストレージに保存
